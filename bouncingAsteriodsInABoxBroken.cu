...
void setInitailConditions()
{
	...
	// ??????????????????????????????????????????????????????????
	// Print out how many meters long each box side is.
	// Print out how many meters/second the max Velocity is.
